...
def polynomial_transform(X, d):
    A = np.asfarray([[None for y in range (d + 1)] for x in range (len(X))])
...
def evaluate_model(Phi, y, w):
    ...
    res = 0 
    for i in range(0, k):
        res = res + (y[i] - w.transpose().dot(Phi[i]))**2
    res = res/k
    return res
...
# From the plot if d vs. validation error below, choice of d as 15, 18, 21 will 
# generalize the best result becuase of the least of validation and test error  
...
lamranges = [0.001, 0.01, 0.1, 1, 10, 100, 1000]

w = {}               # Dictionary to store all the trained models
validationErr = {}   # Validation error of the models
testErr = {}         # Test error of all the models

for lam in lamranges:  # Iterate over polynomial degreel
    Phi_trn = radial_basis_transform(X_trn, X_trn, 0.1)                # Transform training data into d dimensions
    w[lam] = train_ridge_model(Phi_trn, y_trn, lam)                   # Learn model on training data
    
    Phi_val = radial_basis_transform(X_val, X_trn, 0.1)               # Transform validation data into d dimensions
    validationErr[lam] = evaluate_model(Phi_val, y_val, w[lam])  # Evaluate model on validation data
    
    Phi_tst = radial_basis_transform(X_tst, X_trn, 0.1)          # Transform test data into d dimensions
...
plt.xticks(list(validationErr.keys()), fontsize=12)
plt.legend(['Validation Error', 'Test Error'], fontsize=16)
plt.axis([-60, 1000, 25, 60])


#according to the plot, the ideal values of lambda is 0.001 becuase of the least of validation and test error 
